...
class CRDataset(data.Dataset):
    def __init__(self, data_dir, split, is_random_chirp=True, subset=None):
        ...
        self.chirp_ids = [30, 60, 80, 120, 150, 180, 210, 240]
        self.image_paths = []
        self.radar_paths = []
        self.obj_infos = []
        self.confmaps = []
    ...
    def __getitem__(self, index):
        ...
        rawData = np.zeros([128, 8, 8], dtype=np.complex128)   # 8
        if isinstance(chirp_id, list):
            ...
            rawData[:, :, 4] = rawdata[:, :, chirp_id[4]]
            rawData[:, :, 5] = rawdata[:, :, chirp_id[5]]
            rawData[:, :, 6] = rawdata[:, :, chirp_id[6]]
            rawData[:, :, 7] = rawdata[:, :, chirp_id[7]]
            rawData = np.transpose(rawData, (2, 1, 0))  # 4*8*128
            rawData = rawData.reshape((1, 8, 8, 128))   # 8
